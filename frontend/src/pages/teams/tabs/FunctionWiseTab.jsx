...
import { getTeamsFunctionActivity } from '../../../lib/api'
...
export default function FunctionWiseTab({ files, employeeFiles, selectedFileId, setSelectedFileId, selectedEmployeeFileId, setSelectedEmployeeFileId }) {
  ...
  const downloadChartsAsPNG = async () => {
    try {
      ...
      const fileName = `Function_Activity_Charts_${new Date().toISOString().split('T')[0]}.png`
  ...
  const exportToPDF = async () => {
    try {
      ...
      if (chartSections.length === 0) {
        ...
        pdf.text('Function-wise Activity', pdfWidth / 2, 15, { align: 'center' })
      ...
      const fileName = `Function_Activity_Report_${new Date().toISOString().split('T')[0]}.pdf`
  ...
  if (isLoadingFunction || isLoadingFunctionCompare) {
    return (
      <div className="flex items-center justify-center h-96">
        <div className="text-center">
          <div className="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mb-4"></div>
          <p className="text-gray-600">Loading function activity data...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-6 bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 shadow-lg space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white rounded-lg p-3 border-2 border-blue-300 shadow-sm">
            <label htmlFor="teamsFileSelect2" className="block text-sm font-semibold text-gray-800 mb-2 flex items-center gap-2">
          ...
          {groupCompareMode && (
            <div className="bg-white rounded-lg p-3 border-2 border-blue-400 shadow-sm">
              <label htmlFor="groupCompareFileSelect" className="block text-sm font-semibold text-gray-800 mb-2 flex items-center gap-2">
                <span className="lnr lnr-file-empty text-blue-600"></span>
                Teams File 2
              </label>
              <select
      ...
      {functionData.length === 0 && !isLoadingFunction && (
